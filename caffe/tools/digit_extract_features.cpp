...
int main(int argc, char** argv) {
		 ...
		 /*********************************/
			//argv[1]:pretrained model
			//argv[2]:net proto file
			//argv[3]:the layer to extract
			//argv[4]:where to store the output(lmdb or other format)
			//argv[5]:itr_max(data_size/batch_size)
			//argv[6]:db type of argv[4]
			//argv[7]: name of the file to store the output (as .txt&.csv)
		 /*********************************/
...
int feature_extraction_pipeline(int argc, char** argv) {
  ...
  const int num_required_args = 8;     /// the parameters must be not less 8
  ...
  std::vector<std::ofstream*> fcsv(num_features, 0);
  ...
  for (size_t i = 0; i < num_features; ++i) {
	...
	std::string csv_strfile = strfile + index + ".csv";
    vec[i] = new std::ofstream(str.c_str());
	fcsv[i] = new std::ofstream(csv_strfile.c_str());
	(*fcsv[i]) <<"ImageId"<<","<<"Label"<< std::endl;
  }
    ...
    for (int i = 0; i < num_features; ++i) {    // to exact the blobs' name  maybe fc7 fc8
      ...
      for (int n = 0; n < batch_size; ++n) {
		...
		for (int d = 0; d < dim_features; ++d) {
		  ...
		  if(feature_blob_data[label]<feature_blob_data[d])
			  label=d;
        }
		 ImageId=image_indices[i]+1;
		 (*fcsv[i]) <<ImageId<<" ,"<<label<< std::endl;   //csv
        (*vec[i]) << std::endl;
